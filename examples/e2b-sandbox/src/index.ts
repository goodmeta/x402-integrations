/**
 * E2B Sandbox — x402 v1 integration example
 *
 * Wraps the E2B sandbox creation API with x402 payment gating.
 * An AI agent pays a small USDC fee to spin up an isolated code
 * execution environment — no E2B account or API key needed on the agent side.
 *
 * This is an x402 v1 example (same pattern as the Tavily example):
...
interface Env {
  /** Your E2B API key — the upstream service we're wrapping with payments */
  E2B_API_KEY: string;

  /**
   * x402 facilitator URL — the payment verification service.
   * See the Tavily example for detailed explanation of facilitators.
   */
  ...
  /**
   * Price per sandbox in USDC base units (6 decimals).
   * "5000" = 0.005 USDC ($0.005) — adjust based on your E2B costs.
   */
 ...
 * See Tavily example for detailed explanation.
 */
const USDC_BASE = "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913";

/**
 * CAIP-2 network identifier for Base mainnet.
 * "eip155:8453" = Ethereum-compatible chain #8453 (Base).
 */
...
/**
 * Tells the agent: "Pay this much USDC to this address to create a sandbox."
 ...
 * Same as Tavily — v1 verify-only, no on-chain settlement.
 */
 ...
 * x402 v1 format — see Tavily example for v1 vs v2 differences.
 */
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    // Step 4: Payment verified — create the E2B sandbox
